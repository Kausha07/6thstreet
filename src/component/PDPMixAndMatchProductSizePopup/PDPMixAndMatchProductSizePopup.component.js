...
class PDPMixAndMatchProductSizePopup extends PureComponent {
    ...
    renderSizeOption(simple_products, code, label) {
        ...
        return (
            <div block="PDPMixAndMatchProductSizePopup-SizeSelector" elem="SizeOptionContainer" key={ `MixAndMatch-${code}` }>
                <input
                    type="radio"
                    id={ `MixAndMatch-${code}` }
                    block="PDPMixAndMatchProductSizePopup"
    ...
    getSizeSelect() {
        ...
        if (sizeObject.sizeCodes !== undefined
            ...
            return (
                <div block="PDPMixAndMatchProductSizePopup-SizeSelector" elem="AvailableSizes">
                    {
    ...
    renderQuantityBasedMessage(qty) {
        switch (qty) {
            case '0':
                return  (
                    <div block="PDPMixAndMatchProductSizePopup-SizeSelector" elem="QuantityBasedMessage">
                        { __('Out of stock') }
                    </div>
                );
            case '1':

...
export class PDPMixAndMatchProductContainer extends PureComponent {
    ...
    togglePDPMixAndMatchProductSizePopup() {
      this.setState({isSizePopupOpen: !this.state.isSizePopupOpen}, this.toggleRootElementsOpacity);
    }
    ...
    onSizeSelect(e) {
        e.persist();
        this.setState({ selectedSizeCode: e.target.value }, this.togglePDPMixAndMatchProductSizePopup);
    }
    ...
    componentDidMount() {
        ...
        getProductStock(sku).then((response) => {
          ...
          this.setState({ processingRequest: false, mappedSizeObject: object, selectedSizeCode: object.sizeCodes[0] });
        });
      }
        ...
        return (
          <>
            <PDPMixAndMatchProduct
              ...
              { ...this.containerProps() }
            />
            { isSizePopupOpen && <PDPMixAndMatchProductSizePopup {...this.containerFunctions } { ...this.containerProps() } /> }
          </>
        );
    }
}

export default connect(null, mapDispatchToProps)(PDPMixAndMatchProductContainer);

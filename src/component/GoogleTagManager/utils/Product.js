...
class Product {
  ...
  static getList() {
    const meta =
      GoogleTagManager.getEvent(EVENT_GENERAL).currentMeta.metaObject || {};

    return meta.name || meta.title || document.title.split("|").pop() || "";
  }
  ...
  static getItemData(item = {}) {
    if (item && Object.values(item).length) {
      ...
      const configurableVariantIndex = this.getSelectedVariantIndex(
        product,
        sku
      );
  ...
  static getPrice(variant, type_id) {
    const {
      price: {
        minimalPrice: { amount: { value: discountValue = null } = {} } = {},
        regularPrice: { amount: { value = 0 } = {} } = {},
      } = {},
    } = variant;
  ...
  static addGroupedProduct(groupedProductData, product, groupedProductPrice) {
    ...
    if (existingGroupedProduct) {
      const {
        data: { [GROUPED_PRODUCT_PRICE]: oldPrice },
      } = existingGroupedProduct;
      groupedProducts[sku].data[GROUPED_PRODUCT_PRICE] =
        groupedProductPrice + oldPrice;
    } else {
      groupedProducts[sku] = {
  ...
  static updateGroupedProduct(childSku, price) {
    ...
    if (skuOfProductToUpdate) {
      const { [GROUPED_PRODUCT_PRICE]: prevPrice } =
        groupedProducts[skuOfProductToUpdate].data;
      ...
      groupedProducts[skuOfProductToUpdate].data[GROUPED_PRODUCT_PRICE] +=
        price;
      GTMInstance.setGroupedProducts(groupedProducts);
    }
  ...
  static mergeGroupedProducts(groupedProducts1, groupedProducts2) {
    ...
    Object.keys(groupedProducts1 || {}).forEach((key) => {
      if (groupedProducts2[key]) {
        result[key].data[GROUPED_PRODUCT_PRICE] +=
          groupedProducts1[key].data[GROUPED_PRODUCT_PRICE];
      } else {
  ...
  static getVariantSku(sku, variantSku, isVariantPassed) {
    return variantSku === sku && !isVariantPassed ? NOT_APPLICABLE : variantSku;
  }
  ...
  static getAttribute(variant, parentAttributes, attributeName) {
    const { attribute_value = "" } = variant.attributes
  ...
  static getProductData(item) {
    const {
      brand_name,
      color,
      full_item_info: { category = "", qty, size_option, basePrice } = {},
      sku: parentSku,
      name: parentName,
      optionValue,
      row_total,

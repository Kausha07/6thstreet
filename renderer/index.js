...
renderer.get('*', async (req, res) => {
    const locale = req.headers.host.split('.')[0];
    console.log(`http://${locale}.localhost:3000${req.originalUrl}`)
    ...
    if(!html){
        let renderResponse = await chrome.instance(`http://${locale}.localhost:3000${req.originalUrl}`, browserWSEndpoint);
